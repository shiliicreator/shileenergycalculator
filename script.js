...
function renderTable(doCalculation = true) {
  ...
  devices.forEach(device => {
    ...
    // + чекбокс и "крестик" для удаления
  ...
  periods.forEach(period => {
    ...
    devices.forEach(device => {
      ...
      if (doCalculation && device.enabled) {
        // Общие расчёты:
        // Мощность (Вт) -> кВт = device.power / 1000
        // Использование в часах (в день) = device.usageHours
        ...
        switch (period.type) {
          ...
          case 'day':
            // За один день (учитывая, что прибор работает device.usageHours в сутки)
            consumption = powerKwt * device.usageHours;
            cost = consumption * device.pricePerKwh;
            break;

          case 'month':
            // За месяц (30.44 дней) при том же кол-ве часов использования в день
          ...
          case 'custom':
            // "Пользовательское время" (те же device.usageHours часов)
            consumption = powerKwt * device.usageHours;
            cost = consumption * device.pricePerKwh;
            break;
        }
  ...
  // Добавим обработчики для чекбоксов (включение/выключение прибора) и крестиков (удаление)
...
/*
  ...
  1. Если в форме введены новые данные (и они корректны), добавляем прибор в список.
  2. Сразу считаем (renderTable(true)).
*/
calculateBtn.addEventListener('click', () => {
  // Если пользователь что-то ввёл в поля, добавляем прибор
  if (
    ...
    // Если пользователь не вводил новые данные, просто пересчитываем
    renderTable(true);
  }
});
...
/*
  ...
  2. Тут же обновляем таблицу (но вычисление тоже делаем).
*/
